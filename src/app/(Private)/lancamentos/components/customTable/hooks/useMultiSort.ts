 ...
 * };
 *
 * const { sortedData, requestSort, getSortIcon } = useMultiSort(lancamentos, columns);
 *
 * // No onClick do header da tabela:
 * <TableCell onClick={() => requestSort('data')}>Data</TableCell>
 */
      ...
      const newConfigs: MultiSortConfig<T>[] = (() => {
        ...
        if (existingIndex !== -1) {
          ...
          if (existingConfig.order === "ASC") {
            updated[existingIndex] = { key: key as keyof T, order: "DESC" };
            return updated;
          }
          // DESC -> remove
        ...
        return [...sortConfigs, { key: key as keyof T, order: "ASC" }];
      })();
